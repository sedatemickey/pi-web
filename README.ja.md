...
[pi コーディングエージェント](https://github.com/earendil-works/pi)用ローカルブラウザー UI [Pi Web](https://github.com/agegr/pi-web) の拡張版です（[本リポジトリ](https://github.com/sedatemickey/pi-web)）。アップストリームの `main` に追従しつつ、以下の機能強化を加えています。区切り線より下は元の Pi Web の README です。

## このフォークの追加機能

### 宣言型ビジュアルカード（`pi-ui`）

AI の応答には、通常の Markdown に加えて信頼できる構造化ビジュアルコンポーネントを含められます。モデルは `pi-ui` フェンスコードブロック内に小さな宣言型 JSON を出力し、信頼できるレンダラーがそれを 17 種類のカードに変換します：

- **基本** — `metrics`、`comparison`、`steps`
- **インタラクティブ** — `tabs`、`accordion`、`data-table`。キーボード操作可能なタブ、検索・ソート・ページング対応テーブル（CSV ダウンロード付き）
- **チャート** — `bar-chart`、`line-chart`、`area-chart`、`donut-chart`、`sparkline`、`heatmap`。信頼できるレンダラーによる静的描画、ホスト制御のスタイル、スクリーンリーダー向けに視覚的に隠されたデータテーブルを提供
- **情報** — `status`、`key-value`、`progress`、`checklist`、`callout`

プロトコルは厳格なセキュリティと互換性の境界を中心に設計されています：

- すべてのブロックは厳格なスキーマ検証（型・列挙・サイズ・深さ・数の制限）を通過する必要があります。無効または未対応のソースは、壊れたカードではなく元のコードとして表示されます。
- モデルが提供するのは**データのみ**です。HTML、スクリプト、URL、スタイル、色、アイコン、フォーマット関数、アクションは許可されず、外観とインタラクションはすべてホストが制御します。
- すべてのカード種別に Markdown フォールバック、ソース表示、raw / 読み取り可能な 2 種類のコピー、テーブルの TSV/CSV エクスポート、セッション HTML エクスポート時の変換があります。
- カードは応答のストリーミングに合わせて段階的にレンダリングされ、コードフェンスが閉じた時点で表示されます。

### Pi UI 能力設定

新しい **Pi UI** 設定ページで、モデルが生成できるカード種別を制御できます：

- マスタースイッチと 17 コンポーネントそれぞれの独立トグルに、ライブプレビュー、フィールド制約、JSON 例を付属。
- 設定は `~/.pi/agent/pi-ui/settings.json` にグローバル保存され、メイン、Chat-only、復元、サブエージェントの各セッションのシステムプロンプトから、無効化されたカード種別の告知を除外します。
- スイッチは**モデルへのプロンプトにのみ影響**します。セッション履歴にある既存のカードは常にレンダリングされ続けます。
- サーバー側でセッションごとの設定世代を追跡し、新しいプロンプトを反映するためにどのアクティブセッションのリロードが必要か（送信していない新規チャットを含む）を正確に通知します。

完全なワイヤ形式、制限、フォールバック動作、クロスレンダラー要件は [docs/visual-cards.md](./docs/visual-cards.md) に記載されています。
