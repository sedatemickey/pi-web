...
test("restores the last settings section and falls back without a project", () => {
  ...
  setLastSettingsSection("pi-ui", storage);
  assert.equal(getLastSettingsSection(null, storage), "pi-ui");
...
test("shares the models and pi-ui selections globally", () => {
  ...
  assert.equal(getLastSettingsSelection("pi-ui", "/project-two", storage), "bar-chart");
  assert.equal(getLastSettingsSelection("pi-ui", null, storage), "bar-chart");
});
