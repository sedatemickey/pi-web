...
test("normal and exact prompt paths advertise the host visual-card protocol", async () => {
  ...
  assert.match(startupSource, /appendSystemPromptOverride: appendVisualCardPrompt/);
  assert.match(startupSource, /withVisualCardPrompt\(subagentResources\.appendSystemPrompt\[0\] \?\? ""\)/);
  assert.match(startupSource, /withVisualCardPrompt\(contextFilesSystemPrompt\(/);
});
...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(subagentSource, /appendSystemPromptOverride: appendVisualCardPrompt/);
  assert.match(subagentSource, /exactSystemPrompt: withVisualCardPrompt\(promptPlan\.exactSystemPrompt\)/);
  ...
  assert.doesNotMatch(snapshotSource, /VISUAL_CARD|withVisualCardPrompt|appendVisualCardPrompt/);
  ...
  assert.match(startupSource, /appendSystemPromptOverride: appendVisualCardPrompt/);
  assert.match(startupSource, /withVisualCardPrompt\(subagentResources\.exactSystemPrompt!\)/);
  assert.match(startupSource, /withVisualCardPrompt\(contextFilesSystemPrompt\(/);
