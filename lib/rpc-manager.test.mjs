...
test("normal and exact prompt paths advertise the host visual-card protocol", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(startupSource, /appendSystemPromptOverride: appendConfiguredPiUiPrompt/);
  assert.match(startupSource, /withConfiguredPiUiPrompt\(subagentResources\.appendSystemPrompt\[0\] \?\? ""\)/);
  assert.match(startupSource, /withConfiguredPiUiPrompt\(contextFilesSystemPrompt\(/);
});

test("tracks the Pi UI settings generation across startup, reload, and teardown", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const subagentSource = await readFile(new URL("./subagent-runtime.ts", import.meta.url), "utf8");
  const reloadSource = source.slice(source.indexOf("  private async reloadInner"), source.indexOf("  private installExactSystemPromptContinuation"));
  const destroySource = source.slice(source.indexOf("  destroy(): void"), source.indexOf("  async shutdown():", source.indexOf("  destroy(): void")));

  assert.match(source, /this\.piUiSettingsGeneration = options\.piUiSettingsGeneration \?\? getPiUiSettingsGeneration\(\)/);
  assert.match(source, /setPiUiSessionGeneration\(this\.piUiTrackedSessionId, this\.piUiSettingsGeneration\)/);
  assert.match(source, /setPiUiStartingSessionGeneration\(startingSessionId, piUiSettingsGeneration\)/);
  assert.match(source, /clearPiUiStartingSessionGeneration\(startingSessionId\)/);
  assert.match(reloadSource, /const piUiSettingsGeneration = getPiUiSettingsGeneration\(\);[\s\S]*?await this\.inner\.reload\(options\)[\s\S]*?setPiUiSessionGeneration\(this\.piUiTrackedSessionId, piUiSettingsGeneration\)/);
  assert.match(destroySource, /clearPiUiSessionGeneration\(this\.piUiTrackedSessionId\)/);
  assert.match(subagentSource, /const piUiSettingsGeneration = getPiUiSettingsGeneration\(\)/);
  assert.match(subagentSource, /piUiSettingsGeneration,/);
});
...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(subagentSource, /appendSystemPromptOverride: appendConfiguredPiUiPrompt/);
  assert.match(subagentSource, /exactSystemPrompt: promptPlan\.exactSystemPrompt/);
  assert.match(source, /exactSystemPrompt: \(\) => withConfiguredPiUiPrompt\(options\.exactSystemPrompt!\)/);
  const snapshotSource = subagentSource.slice(
    subagentSource.indexOf("resourceSnapshot:"),
    subagentSource.indexOf("sessionManager.appendCustomEntry", subagentSource.indexOf("resourceSnapshot:")),
  );
  assert.doesNotMatch(snapshotSource, /VISUAL_CARD|withConfiguredPiUiPrompt|appendConfiguredPiUiPrompt/);
  ...
  assert.match(startupSource, /appendSystemPromptOverride: appendConfiguredPiUiPrompt/);
  assert.match(startupSource, /withConfiguredPiUiPrompt\(subagentResources\.exactSystemPrompt!\)/);
  assert.match(startupSource, /withConfiguredPiUiPrompt\(contextFilesSystemPrompt\(/);
...
test("exact prompts keep their startup snapshot until reload succeeds", async () => {
  let configuredPrompt = "prompt-v1";
  const agent = {
    state: {},
    prepareNextTurnWithContext: async (turn) => ({ context: turn.context }),
  };
  const inner = {
    sessionId: "exact-prompt-snapshot",
    isStreaming: false,
    isCompacting: false,
    isBashRunning: false,
    sessionManager: { getCwd: () => process.cwd() },
    settingsManager: { getDefaultTools: () => [], setProjectTrusted: () => {} },
    agent,
    extensionRunner: {},
    getActiveToolNames: () => [],
    getAllTools: () => [],
  ...
  const wrapper = new AgentSessionWrapper(inner, {
    exactSystemPrompt: () => configuredPrompt,
  });

  try {
    assert.equal(agent.state.systemPrompt, "prompt-v1");
    configuredPrompt = "prompt-v2";
    wrapper.setActiveToolSelection([]);
    const beforeReload = await agent.prepareNextTurnWithContext({ context: { systemPrompt: "sdk" } });
    assert.equal(agent.state.systemPrompt, "prompt-v1");
    assert.equal(beforeReload.context.systemPrompt, "prompt-v1");

    await wrapper.send({ type: "reload" });
    const afterReload = await agent.prepareNextTurnWithContext({ context: { systemPrompt: "sdk" } });
    assert.equal(agent.state.systemPrompt, "prompt-v2");
    assert.equal(afterReload.context.systemPrompt, "prompt-v2");
  } finally {
    wrapper.destroy();
  }
});
...
test("reloading a session invalidates the models cache", async () => {
  ...
  const reloadHelper = source.slice(
    source.indexOf("  private async reloadInner"),
    source.indexOf("  private installExactSystemPromptContinuation"),
  );

  assert.match(reloadSource, /await this\.reloadInner\(\)/);
  assert.match(reloadHelper, /await this\.inner\.reload\(options\)/);
  assert.match(reloadSource, /this\.applyExactSystemPrompt\(\);[\s\S]*?invalidateModelsCache\(\)/);
});
