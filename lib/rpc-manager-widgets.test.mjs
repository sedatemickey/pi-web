...
test("command-context reload clears widgets and applies the current Pi UI generation", async () => {
  ...
  advancePiUiSettingsGeneration();
  assert.equal(isPiUiSessionPromptStale(wrapper.sessionId), true);
  await wrapper.createExtensionCommandContextActions().reload();
  assert.equal(disposed, 1);
  assert.deepEqual(await snapshot(wrapper), []);
  assert.equal(isPiUiSessionPromptStale(wrapper.sessionId), false);
  wrapper.destroy();
});
